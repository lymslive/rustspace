...
// cargo test -F toml (or --all-features)
...
fn path_test() {
    ...
    assert_eq!(v.pathto("ip").unwrap().as_str(), Some("127.0.0.1"));
...
fn pipe_test() {
    ...
    assert_eq!(value, 1234);
    let value = v.pathto("misc/int/") | 0;
    assert_eq!(value, 0); //< tail / means an extar empty string key
    let value = v.pathto("misc.int") | 0;
    assert_eq!(value, 1234);
    let value = v.pathto("/misc/./int/") | 0;
    assert_eq!(value, 0);
...
fn push_test() {
    ...
    // push mistype value silently overwrite
    let ip_node = v.path_mut() / "ip";
    let ip_node = ip_node << 127;
    assert_eq!(ip_node.is_none(), false);
    let ip = v.path() / "ip" | "";
    assert_eq!(ip, "");
    let ip = v.path() / "ip" | 0;
    assert_eq!(ip, 127);
    let ip = v.path() / "ip" | "".to_string();
    assert_eq!(ip, "127");
